...
**DONE (1 of 7):**
- Item 13: R&D capitalization per Damodaran's R&DConv.xls. Pre-tax research asset with straight-line amortization, industry-specific amortizable life (3yr software, 5yr semi, 10yr pharma). First pass (2026-04-09): adjusted ROIC flows into calibrate coherence checks and terminal ROIC anchoring. Second pass (2026-04-11): adjusted operating margin and adjusted S/C also fed calibrate step 6b/6c anchors and DCF starting-point extraction. Third pass (2026-04-11 late): after post-ship review surfaced a tax-treatment mixing issue (DCF's `× (1 − t)` applied to a pre-tax adjusted margin produced `(EBIT + delta)(1-t) = EBIT(1-t) + delta(1-t)`, undercounting NOPAT by `t × delta` per year vs Damodaran's canonical pre-tax add-back `EBIT(1-t) + delta`), reframed the fix from algorithmic to informational. Adjusted values are now display-only reference anchors in calibrate and value; DCF runs on user picks with raw GAAP fallback when uncalibrated. Calibrate step 6b now asks explicitly for starting margin (previously only target). Step 7 opens with a ROIC panel showing raw/adjusted/user-implied/terminal in one place. See `docs/internal/2026-04-11-rd-informational-design.md` and `docs/internal/2026-04-11-rd-informational-plan.md`.
