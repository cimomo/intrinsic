...
- **Cost of Debt:** Derive from credit rating and Damodaran's default spread table.
  ...
  2. **Web search** for the actual credit rating. Rating agencies publish ratings in many places, and older articles rank high — a naive search almost always returns a stale rating, not the latest action. Use a time-targeted query: search `"{company_name} credit rating latest action upgrade downgrade {current_year-1} {current_year}"`. If the first search returns a rating but no clear date or agency action context, do a **verification search**: `"{company_name} credit rating {found_rating} {current_year}"` to confirm it is current. Extract the rating if clearly stated with a recent date (e.g., "Aaa", "AA+").
